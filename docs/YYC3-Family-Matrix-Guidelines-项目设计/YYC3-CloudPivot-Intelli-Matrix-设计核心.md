...
智能终端切换系统实现了两种布局模式下终端面板的自适应位置调整，确保用户在不同工作流程中获得最优的屏幕空间利用率。

### 布局模式定义

#### 编辑模式 (Edit Mode)

终端仅在右栏（代码编辑器下方）显示，中栏文件资源管理器保持全高度，适合文件浏览和代码编辑并行操作。
...
终端跨越中栏+右栏显示，提供更宽的终端视野，适合命令行密集操作和日志查看。

```
┌──────────┬──────────────────────────────────────┐
│ AI Chat  │ File Explorer   │ Code Editor        │
│ (25%)    │ (40% of 75%)    │ (60% of 75%)      │
│          ├──────────────────┴────────────────────┤
│          │ Terminal (跨越中栏+右栏)               │
└──────────┴──────────────────────────────────────┘
```
...
interface IDELayoutState {
  layoutMode: IDELayoutMode;     // 'edit' | 'preview'
  viewMode: IDEViewMode;         // 'default' | 'preview' | 'code'
  terminalCollapsed: boolean;    // 终端是否折叠
}

// 布局模式类型
type IDELayoutMode = "edit" | "preview";

// 视图模式类型
type IDEViewMode = "default" | "preview" | "code";
```
...
| 操作 | 快捷键 | 效果 |
| ------ | -------- | ------ |
| 切换终端显示/隐藏 | `Ctrl + \`` | 终端折叠/展开，布局模式不变 |
| 切换布局模式 | `Ctrl + 3` | 编辑模式 ↔ 预览模式，终端显示状态保持不变 |
| 切换预览视图 | `Ctrl + 1` | 隐藏/显示中栏文件资源管理器 |
| 切换代码视图 | `Ctrl + 2` | 隐藏/显示左栏 AI 对话面板 |
| 全局搜索 | `Ctrl + Shift + F` | 打开/关闭搜索覆盖层 |
| 关闭搜索 | `Esc` | 关闭搜索覆盖层 |

### 终端宽度计算

```typescript
function calculateTerminalWidth(layoutMode: IDELayoutMode, windowWidth: number): number {
  if (layoutMode === 'edit') {
    return windowWidth * 0.3;   // 仅右栏宽度
  } else {
    return windowWidth * 0.75;  // 中栏+右栏合并宽度
  }
}

function calculateTerminalLeft(layoutMode: IDELayoutMode, windowWidth: number): number {
  if (layoutMode === 'edit') {
    return windowWidth * 0.7;   // 左栏25% + 中栏45%
  } else {
    return windowWidth * 0.25;  // 左栏宽度
  }
}
```
...
```typescript
function adjustTerminalHeight(newHeight: number): number {
  return Math.max(100, Math.min(400, newHeight));
}
```

- **最小高度**: 100px（折叠状态为 28px 标题栏）
- **最大高度**: 400px
- **默认高度**: 面板总高度的 30%
...
├── ide-types.ts          # IDELayoutMode、IDEViewMode 等类型定义
├── ide-mock-data.ts      # Mock 数据（文件树、聊天记录、AI 模型）
├── IDETopBar.tsx         # 顶部导航栏（Y3 Logo + 项目名 + 图标组 + 用户头像）
├── IDEViewSwitcher.tsx   # 视图切换栏（返回/预览/代码/布局模式/搜索/更多）
├── AIChatPanel.tsx       # 左栏 AI 对话面板（模型选择 + 聊天 + 输入）
├── FileExplorer.tsx      # 中栏文件资源管理器（文件树 + 搜索过滤）
├── CodePreviewPanel.tsx  # 右栏代码编辑器（多 Tab + CodeMirror）
├── IDETerminal.tsx       # 集成终端（多 Tab + 命令行 + 历史记录）
└── IDELayout.tsx         # 主布局组件（编辑/预览模式条件渲染）
```

### 实现要点

1. **条件面板结构**: `IDELayout` 根据 `layoutMode` 条件渲染两套不同的 `react-resizable-panels` 嵌套结构
2. **状态独立**: `layoutMode` 与 `viewMode` 是正交状态，互不影响
3. **终端实例复用**: 终端组件在两种布局模式间共享状态，切换布局不会重置终端内容
4. **i18n 完备**: 所有文案使用 `t("ide.xxx")` 调用，支持 zh-CN / en-US 双语
5. **面板 ID 分离**: 编辑模式和预览模式使用不同的 Panel ID（`-e` / `-p` 后缀），避免 `react-resizable-panels` 的 ID 冲突

### 赛博朋克视觉规范

| 元素 | 样式 |
| ------ | ------ |
| 面板背景 | `rgba(4,10,22,0.6)` ~ `rgba(4,10,22,0.8)` |
| 拖拽手柄 | `rgba(0,212,255,0.15)` hover `rgba(0,212,255,0.4)` |
| 边框分隔线 | `rgba(0,180,255,0.08)` ~ `rgba(0,180,255,0.12)` |
| 强调色 | `#00d4ff`（青色） |
| 成功色 | `#00ff88`（绿色） |
| 错误色 | `#ff3366`（红色） |
| 文本主色 | `#e0f0ff` |
| 文本辅色 | `rgba(0,212,255,0.3)` ~ `rgba(0,212,255,0.5)` |
| 布局模式指示条 | 编辑模式 `rgba(0,212,255,0.05)` / 预览模式 `rgba(0,255,136,0.05)` |
