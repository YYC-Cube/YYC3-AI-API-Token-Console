...
| 首轮遗留 | 处置状态 |
| -------- | -------- |
| CORS 按网段收敛（`ALLOW_ORIGIN` 环境变量化） | ⬜ 仍开放（内网部署场景，P1） |
...
1. **[P1]** CORS 按网段收敛（`deploy/server.mjs` `ALLOW_ORIGIN` 环境变量化）— 首轮遗留，30 分钟
...
1. **[P1]** CORS 网段收敛 — 首轮唯一遗留 P1
2. **[P1]** lint warnings 渐进治理
3. **[P2]** 大文件拆分 + 2026-12 Phase 4 触发条件季度核对
